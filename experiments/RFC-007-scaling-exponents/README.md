# RFC-007 — The Scaling Exponents δ and γ, and Their Trajectories

**Status:** Awaiting RFC. `proposals/RFC-007-scaling-exponents.md` has not been written yet.
**Source:** RFC-001 §8.3.
**Feeds:** RFC-008 (§8.2), which uses these exponents for its domain-specific boundary test.
...
**Reuse opportunity:** this experiment's own method calls for "natural corpora across
domains with varying clustering strength" — which is exactly what RFC-002 through RFC-006
will have already produced (semantic-layer/text-to-SQL, embeddings, agentic memory, code,
and whatever $\epsilon$ operationalization RFC-006 settles on). Check whether this can be
a meta-analysis over those four datasets before designing fresh corpus collection.
...
Freeze `proposals/RFC-007-scaling-exponents.md` first — it needs to specify the domain
