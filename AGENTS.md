...
(RFC-001, "Resonance"). RFC-001 proposes six experiments in its §8; each gets its own RFC
in `proposals/` and its own execution folder in `experiments/`.

## RFC conventions

- **Naming:** `proposals/RFC-0NN-<kebab-slug>.md`, numbered sequentially.
- **Frozen means frozen.** RFC-001 states its own frozen status explicitly ("Status of
  ...
  the version label.
- **An experimental RFC (002–007) must close the gaps RFC-001 leaves open** for that
  experiment before it can be frozen: effect sizes, the model-comparison/null-model
  criterion, corpus-size or power requirements, and (for RFC-007 specifically) the
  §8.6.1 prior-art survey.
...
Literature-survey requirements (e.g. RFC-007's §8.6.1 prior-art check) should be treated
as real blocking work, not a formality — confirm the framing isn't already occupied
before freezing the RFC that depends on it.
